...
resize_size = 512
n_flows = 8
output_dir = "./assets/custom_flows_apple_test_test_menu"
...
while running:
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            # --- Display GUI for parameter selection ---
            ...
            def submit():
                ...
                for k in entries:
                    ...
                    if k == 'prompt':
                        ...
                        yaml_data[k] = float(val) if '.' in val else int(val)
            ...
            tk.Button(root, text="Save Config", command=submit).grid(row=len(defaults), column=0, columnspan=2)
            root.mainloop()
