...
def main():
    ...
    print("Flow are placed in the data directory for easy compatibility!")
    flow_dir.mkdir(parents=True, exist_ok=True)  # create if it doesn't exist
    for idx, flow in enumerate(flows):
        save_path = flow_dir / f"flow_{idx:04}.pth"
        torch.save(flow.cpu(), save_path)
    print(f"Saved {len(flows)} flows to {flow_dir}")

    # EXTRACT FLOWS
    flow_files = sorted([f for f in flow_dir.glob('*.pth') if f.name.startswith('flow_')])
    if not flow_files:
        print(f"No flow_*.pth flow files found in {flow_dir}")
        return
